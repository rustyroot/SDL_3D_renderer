...
physical_point_t* create_physical_point (point_t position, float charge, float mass) {
    ...
    point->speed = (point_t){0, 0, 0};
    point->acceleration = (point_t){0, 0, 0};
    point->charge = charge;
    point->mass = mass;
...
void update_physical_point (physical_point_t point, point_t* forces, int nb_forces) {
    ...
    for (int i = 0; i < nb_forces; i++) {
        point.acceleration = somme_point(point.acceleration, produit_par_scalaire(1/point.mass, forces[i]));
    }
    point.speed = somme_point(point.speed, produit_par_scalaire(time_since_last_update, point.acceleration));
    point.position = somme_point(point.position, produit_par_scalaire(time_since_last_update, point.speed));
}
